...
模板为C\++提供了鸭子类型（Duck typing）的特征。所谓鸭子类型，指的是代码关注的不是对象的类型本身，而是它被如何使用的。例如，在使用鸭子类型的语言中，我们编写一个函数可以接受一个任意类型的对象，只要它有走、游泳和嘎嘎叫方法。至于客户给它传入的是一只真正的鸭子，或是也能走、游泳和嘎嘎叫的其它类型对象，都没有关系。但是如果传入的对象中没有这些需要被调用的方法，就将引发一个错误。
...
对`max`，只约束入参类型T需要支持`>`比较运算，至于它的具体类型是不关心的。例如我们可以为`max`传入`int`、`float`或者是实现了了`opertator>`的任何对象。

直到现在，C\++中模板对入参的约束都是通过对入参的使用来隐式约束的。而在有的语言里面，却可以显示约束。例如对于如下Haskell代码：
...
如上`max`的定义前面有一句函数声明：`max' :: (Ord a) => a -> a -> a`，它约束了函数的入参类型`a`必须满足类型类`Ord`的约束。`Ord`作为一个类型类，用于规范一组类型应该满足的特征，它要求满足它的类型必须能够进行标准的比较操作，如`<`、`>`、`<=`、`>=`等。

C\++17标准有可能会引入concept特征，该特征类似上述haskell中对类型特征进行显示约束的能力。显示约束可以让代码更容易被理解，让编译器可以更准确的报告错误或者对代码更好地做出优化，同时也可以让IDE对语言的支持变得更好。

鸭子类型为程序的书写带来了很多便利性，基本上动态语言（Python、Ruby）以及拥有类型推导的静态语言（C\++、Haskell、Scala）都有这个特性。区别在于动态语言一般要在运行期才能发现类型不满足约束，而静态语言通过强大的类型推导可以在编译期就发现错误。

那么回到最后，我们思考下，如果把C\++模板元编程当做一门独立的语言，它自身是否支持鸭子类型，它又是动态语言还是静态语言呢？

首先很明确的，虽然模板为运行时C\++提供了鸭子类型的能力，但模板元编程自身却不支持鸭子类型。

比如 `template<int Value> struct IntType{...}`元函数明确要求其入参为`int`类型变量，所以你可以这样使用`IntType<5>`。但是一旦你传入一个类型给它`IntType<int>`，它就会报错。

我们之前总结过，模板可以操作的计算对象大体可以分为数值和类型两大类，一旦我们把模板当做编译期函数来看，就会发现它是强类型的。一个模板声明其入参是数值，就不能接收类型作为入参，反之亦然！这就是为何我们为了提高元函数的组合复用能力，将所有的数值也封装成了类型。我们统一模板元编程的计算对象类型，就相当于把一切都变成了鸭子，间接地也得到了鸭子类型的好处。

接下来一个问题更有趣，C\++模板元编程自身到底是一门静态语言还是动态语言？
