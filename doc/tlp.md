...
结合这些武器，让通过C\++构建兼顾语法表达力及运行时效率的DSL成为了可能。可以说，模板元编程是上述所有武器中最为重要的，接下来我们通过一个描述有限状态机(FSM)的例子来展示如何使用模板元编程来设计DSL。该例子来自于《C\++模板元编程》一书，原书中作者使用boost中的mpl库来实现。为了让这个例子更加易懂并且代码更加清晰，我用TypeList对其进行了改写。
