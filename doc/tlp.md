...
当1994年，Erwin Unruh在C\++标准委员会上演示了通过C\++模板在编译期计算素数的程序后，C\++模板元编程的能力就已经被人们认识到了。但是由于函数式编程和广大程序员所熟悉的命令式编程存在巨大差异，以及C\++标准和C\++编译器对模板元编程支持上的很多漏洞，导致了模板元编程的代码在一般程序员的眼中十分的晦涩难懂。所以随后的很多年，模板元编程都被大多数C\++程序员认为是一种奇技淫巧。
...
正是如此，C\++标准库STL中就使用了非常多的模板元编程技巧，而且也给程序员提供了很多好用的模板元编程工具，例如我们之前提到的丰富的`type_traits`工具集。而boost中的mpl，则是一个完全为模板元编程打造的库。它实现了全面的用于模板元编程的数据结构和算法，以及各种常用的元编程模式。Mpl甚至还实现了一套编译期的lambda架构，用于在模板元编程中使用编译期的lambda表达式。但是遗憾的是，mpl的实现过程中同时大量的使用了boost的预处理元编程技术，导致代码非常的晦涩难懂，对想通过阅读mpl源码来学习模板元编程的同学增加了非常大的噪音和干扰。

自从C\++11标准发布后，C\++语言和主流编译器对模板以及编译期计算的支持已经逐渐完备，同时标准修复了不少漏洞，使得模板元编程的代码可以写的更加地简单自然。为此，我基于C\++11标准实现了一套针对C\++模板元编程的基础库TLP。这个库的主要目的是为了让大家通过它来学习C\++模板元编程，所以我在写的过程中不断地追求代码的表达力和清晰性。为了能够通过测试用例来展示库组件的用法，我专门编写了一个面向模板元编程进行编译期测试的xUnit测试框架，TLP库中的所有组件都经过了该测试框架的测试。

虽然你软件可以直接依赖整个TLP库，但是对于C\++这么一门让程序员最求极致的语言，学会TLP中模板元编程的设计技巧，然后按需所取地去解决自己的问题显得更为实际。而且模板元编程的特点决定了它不应该出现在你的软件的每一处，而应该只是在最核心地方去解决一些关键的问题。为了让大家能够更好地学习TLP库中的各个组件的设计，我写了这篇手册，介绍了模板元编程的基础知识以及TLP库中关键组件的实现技巧，最后还列举了模板元编程的几个应用。在这个手册中，我提到了很多对编程语言和软件设计的理解，希望也会对大家有用。

正如我最开始所说，模板元编程是中级C\++程序员迈向高级的必经之路，希望本文以及TLP库中的代码对大家更好地学习模板元编程起到帮助，使得我们可以用C\++更有效、更酷的方式去解决问题。
