...
本节开始我们通过使用C\++模板元编程去解决一些实际问题，来展示模板元编程针对现实问题的具体使用和设计技巧。过程中我们会用到TLP库中的一些组件，大多都已经在前文中提到过，请及时对照查阅。

### 数学计算

我们一直强调把C\++模板元编程当做一门全新的图灵完备的纯函数式语言来学习，为了证明它这种能力，之前举的都是类似计算阶乘的简单例子，这里我们通过模板元编程的编译期计算能力完成一道相对复杂但是有趣的数学题目。通过这道题目，我们同时示例了如何做函数式程序设计及领域建模。
...
如上三角形中，我们看到的有点，线以及由线构成的二维图形。针对领域问题建模的时候我们需要选取对解决问题有用的领域概念和属性。那么哪些概念和属性对领域问题有用呢？这时我们采用自顶向下分析，根据目标问题逐级降解问题，最后得到最底层的构建元素，然后再通过自低向上组合来解决问题。
...
- `connected(P1, P2)`：判断两个点是否相连。这其实就是判断两个点是否在同一条直线上，针对我们前面对点和线的属性定义，这其实就是在判断由两个点组成的集合是否为一条线上所有点的集合的子集的算法。
...
TEST("count triangles")
{
    ...
    ASSERT_EQ(__count_triangles(TriplePoints, Figure), __int(24));
};
~~~

上面测试用例中我们先获得所有三个点的组合`using TriplePoints = __comb(Points, __int(3))`，然后把所有的三个点的列表和Figure传递给`__count_triangles`，让它帮我们计算出其中合法的三角形个数。`__comb()`已经被TLP库实现了，我们直接拿来用。

> - `__comb()`：入参为list和一个`__int(N)`，返回由list对于N的所有排列组合的列表；

对于`__count_triangles`的实现应该是遍历每一个`TriplePoints`的成员，对其调用`__is_triangle`，对满足要求的进行累计。对一个列表进行累积的通用算法是`fold`，TLP库中已经有了针对TypeList的fold算法实现了:
...
至此，我们基本上已经实现了这个程序，所有的计算都在C\++编译期完成，我们通过测试用例对结果进行了校验。
